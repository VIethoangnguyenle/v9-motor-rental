# V9 Motor Rental — kiến trúc và ràng buộc
...
> **Tách khỏi `CLAUDE.md` ngày 2026-09-01.** `CLAUDE.md` từ nay **chỉ giữ quy trình làm việc** mà
> mọi agent — kể cả subagent — phải theo. Toàn bộ thông tin về hệ thống, kiến trúc và ràng buộc
> phiên bản chuyển sang file này. Nội dung dưới đây giữ nguyên văn, không cắt xén.
>
> Nạp file này khi cần: hiểu kiến trúc, đụng schema, đụng version, đọc perf budget, hoặc trước khi
> đề xuất một thay đổi không tầm thường.
...
Luật: **trước khi đọc file hay grep để hiểu một vùng code, gọi CodeGraph trước.** Một lời gọi trả
về source verbatim đánh số dòng (dùng thẳng để sửa, không cần `Read` lại), cạnh gọi hàm — kể cả
hop dynamic dispatch mà grep không đi theo được — và blast radius của thứ sắp đụng. Đọc file bằng
tay là làm lại việc index đã làm rồi, đắt hơn cho cùng một câu trả lời.

Ranh giới với Serena: CodeGraph để **hiểu và định vị**; Serena để **sửa theo ngữ nghĩa**. Đổi tên
hay đổi signature của exported function / shared type vẫn bắt buộc `find_referencing_symbols` —
blast radius của CodeGraph là để định hướng, không phải danh sách reference đủ để refactor an toàn.

Hai đường gọi, cùng một output: MCP `codegraph_explore`, hoặc CLI `codegraph explore "<câu hỏi>"`.
**MCP khai trong `.mcp.json` là project scope nên máy mới phải duyệt một lần** — chưa duyệt thì tool
không tồn tại trong phiên, và đường CLI vẫn chạy. Không duyệt được cũng không phải cớ để quay lại
grep.

⚠️ **Đo lại 2026-08-31 trên v1.6.0: hai trong ba lỗi cũ đã hết.** Đừng chép lại cảnh báo cũ từ trí
nhớ. `explore` với tên không tồn tại nay báo thẳng `No relevant code found` (v1.5.0 trả về symbol
không liên quan một cách tự tin) · cờ test đổi từ `⚠️ no covering tests found` sang
`no tests found within 3 caller hops`, tức đã là reachability theo caller hop chứ không còn là
heuristic "tên có xuất hiện trong file test" — `Rental` từ chỗ bị báo oan nay ra đúng
`tested via callers: rentals.test.ts`.

Vẫn phải đọc đúng chữ: **"trong 3 hop" không đồng nghĩa "không có test"**, và hai điều sau còn
nguyên — cạnh gọi hàm **có thể** sai khi trùng tên (cặp `rentals` route-vs-bảng đã kiểm là đúng,
nhưng một cặp sạch không chứng minh được cả lớp) · và **index cũ không cảnh báo gì**: watcher chỉ
sống khi daemon/MCP chạy, nên dùng CLI trần thì `codegraph sync -q` trước.

Sau **mỗi** `codegraph upgrade`, rebuild rồi chạy lại đúng bốn probe này — luật "hàng rào phải được
probe" áp cho cả tool đọc code, không riêng linter:
...
Probe thứ tư chỉ có giá trị nếu bạn **đối chiếu** caller nó liệt kê với `grep` thật. Nó chạy xong
và in ra một bảng đẹp **không chứng minh điều gì**.

`.claude/CLAUDE.md` do `codegraph install --refresh` sinh ra giữa cặp marker `CODEGRAPH_*` và bị
ghi đè mỗi lần nâng version. Nó **phải được commit — đừng đẩy vào `.gitignore`**: có track thì lần
ghi đè sau hiện ra thành diff review được, còn ignore thì nó mọc lại thành file untracked sau
**mỗi** lần upgrade, mãi mãi. Đừng sửa tay phần giữa marker; luật của repo nằm ở file này.
...
| Luật "CodeGraph trước khi đọc"    | `CLAUDE.md` (đã commit)                             | ⚠️ có mặt — **không máy nào ép**   |
...
Luật CodeGraph là **loại thứ ba**, đừng nhầm sang hai loại kia: nó đã commit nên sống sót qua
`git clone`, nhưng không linter nào kiểm được "đã hỏi CodeGraph chưa". Cùng hạng với luật định
danh tiếng Anh — quy ước trong tài liệu, không phải hàng rào.
...
**(d)** `codegraph explore` vùng sắp đụng, rồi implement bằng Serena →
