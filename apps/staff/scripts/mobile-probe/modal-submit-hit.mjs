...
await wait(3000);
await ev(
  `[...document.querySelectorAll('button')].find(b=>b.textContent.includes('Lên đơn'))?.click()`,
);
await wait(2500);
...
function render(result) {
  ...
  if (result.dialog)
    out.push("  dialog: có, open=" + result.dialog.open + " modal=" + result.dialog.modal);
  return out.join("\n");
}
...
if (!selfTest) {
  ...
  console.warn(
    "== --self-test bước 2/3: SAU khi che 30% đáy nút (kỳ vọng: các điểm fy=0.85/0.98 trượt) ==",
  );
